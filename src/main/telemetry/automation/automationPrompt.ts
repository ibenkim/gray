...
- NEVER invent literalText for free-form typing. It must appear in actions[].typedText, actions[].searchQuery, actions[].clipboard.text, step.inputLiteral, OR as a concrete destination/document name already present in workflow.steps[].action / documentTitle. If none exist → ask_user or manual.
- Prefer step.inputVariableKey over inventing a variableKey. Prefer step.inputLiteral when it matches allowed evidence.
...
- Include wait_for after open_app / open_url when useful, and whenever completionCheck/expectedChange/waitedMs imply a wait.
